...
}, function(e, t, n)
{
	...
	function i()
	{
		...
		try
		{
			e = JSON.parse(b.readFileSync(_, "utf8"))
				.bounds
		}
		catch (e)
		{}(w = new p.BrowserWindow(o(
		{
			height: 603,
			width: 850,
			minWidth: 850,
		...
		.setMenu(null), w.webContents.on("will-navigate", (function(e)
		{
		...
		{
			"darwin" === process.platform && p.app.dock.show(), w.webContents.send("window-event", "show"), w.setBounds(a(w.getBounds()))
		})), w.on("close", (function(e)
		{
			if(p.app.isQuiting) p.globalShortcut.unregisterAll(), p.app.exit();
			else
			{
				e.preventDefault(), w.hide(), "darwin" === process.platform && p.app.dock.hide();
				try
				{
					b.writeFileSync(_, JSON.stringify(
					{
						bounds: w.getBounds()
					}))
				}
			...
			return !1
		})), w.on("session-end", (function(e)
		{
			e.preventDefault(), w.webContents.send("app-exit")
					...
					for(;;) switch (e.prev = e.next)
					{
						...
						case 3:
							...
							return o = {
								...
								buttons: ["Reload", "Exit"]
							}, e.next = 7, p.dialog.showMessageBox(w, o);
						case 7:
		...
		}()), w.setTouchBar(new h(
		{
			items: [new m(
			{
				...
				click: function()
				{
					w.webContents.send("menu-item-change", "general")
				}
			}), new m(
			{
				label: "Proxies",
				...
				click: function()
				{
					w.webContents.send("menu-item-change", "proxy")
				}
			}), new m(
			{
				label: "Profiles",
				...
				click: function()
				{
					w.webContents.send("menu-item-change", "server")
				}
			}), new m(
			{
				label: "Logs",
				...
				click: function()
				{
					w.webContents.send("menu-item-change", "log")
				}
			}), new m(
			{
				label: "Connections",
				...
				click: function()
				{
					w.webContents.send("menu-item-change", "connection")
				}
			}), new m(
			{
				label: "Settings",
				...
				click: function()
				{
					w.webContents.send("menu-item-change", "setting")
				}
			}), new m(
			{
				label: "Feedback",
				...
				click: function()
				{
					w.webContents.send("menu-item-change", "about")
				}
			})]
		})), p.powerMonitor.on("suspend", (function()
		{
			w.webContents.send("power-event", "suspend")
		})), p.powerMonitor.on("resume", (function()
		{
			w.webContents.send("power-event", "resume")
		}));
		...
		(E = new p.Tray("darwin" === process.platform ? t : r))
		.setToolTip("Clash for Windows"), E.on("click", (function()
		{
			w.setBounds(a(w.getBounds())), w.show()
		...
		})), p.ipcMain.on("status-changed", (function(e, t)
		{
			try
			{
				"darwin" !== process.platform && E.setImage(t)
			}
			catch (t)
			{}
		...
		},
		{
			...
			{
				var t = e.checked;
				w.webContents.send("system-proxy-changed", t)
			}
		},
		{
			label: "Mixin",
			...
			{
				var t = e.checked;
				w.webContents.send("mixin-changed", t)
			}
		},
		{
			label: "Proxy Mode",
		...
		})), p.ipcMain.on("speed-update", (function(e, t, n)
		{
			try
			{
				if(E)
				{
					...
					r.setTemplateImage(!0), E.setImage(r)
				}
			}
			catch (t)
			{}
		})), E.on("right-click", (function()
		{
			E.popUpContextMenu(i)
		}));
		var c = [
		{
			label: p.app.name,
	...
	n(10)
		...
		.replace(/\\/g, "\\\\"), v(), p.app.disableHardwareAcceleration(), p.app.commandLine.appendSwitch("disable-features", "OutOfBlinkCors"), "darwin" === process.platform && p.app.dock.hide();
	var w, E, _ = g.join(p.app.getPath("userData"), "window_ocnfig.json"),
		O = "file://".concat(__dirname, "/index.html"),
		R = p.app.requestSingleInstanceLock();
	...
	{
		w.webContents.send("app-open", [t])
	})), R ? (p.app.on("second-instance", (function(e, t)
	{
		w && (w.webContents.send("app-open", t), w.isMinimized() && w.restore(), w.show())
	})), p.app.on("ready", (function()
	{
		p.powerMonitor.on("shutdown", (function(e)
		{
			e.preventDefault(), w.webContents.send("app-exit"), setTimeout((function()
			{
				p.app.isQuiting = !0, p.app.quit()
			}), 5e3)
		})), i()
...
}, function(e)
{
	var t = function(e)
	{
		...
		}, c(B), t(B, E, "Generator"), B[v] = function()
		{
			return this
		}, B.toString = function()
		{
